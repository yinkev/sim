 ...
 *
 * Auth validation only applies when **enabling** (`isActive: true`): personal
 * deployments support public and password-protected links, and `password`
...
export async function upsertFileShare({
  ...
  if (isActive) {
    if (finalAuthType === 'email' || finalAuthType === 'sso') {
      throw new ShareValidationError(
        'Email and SSO file sharing are disabled for this personal deployment'
      )
    }
